 ...
 * AI-ассистент продюсера - анализ данных, рекомендации, чат с ПРОМО.АИ
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    if (result.success) {
      ...
      const errMsg: ChatMessage = {
        id: `e-${Date.now()}`,
        role: 'assistant',
        content: 'Произошла ошибка при обращении к ПРОМО.АИ. Попробуйте ещё раз.',
        timestamp: new Date().toISOString(),
        model: 'error',
      };
      ...
      <div className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden flex flex-col" style={{ minHeight: '500px', maxHeight: 'calc(100vh - 280px)' }}>
                ...
                <div className="flex items-center gap-2 mt-1 px-1">
                  ...
                  {msg.model && msg.model !== 'error' && (
                    <span className="text-[9px] text-purple-500/60 bg-purple-500/5 px-1.5 py-0.5 rounded-full">
                      {msg.model === 'fallback' ? 'шаблон' : 'ПРОМО.АИ'}
                    </span>
                  )}
                </div>
              </div>
        ...
        <div className="border-t border-white/5 p-3 sm:p-4">
          <div className="flex items-end gap-2">
            <div className="flex-1 relative">
              <textarea
                ...
                placeholder="Задайте вопрос ПРОМО.АИ..."
                rows={1}
          ...
          <p className="text-[10px] text-gray-600 mt-2 text-center">
            ПРОМО.АИ анализирует ваши данные и даёт персональные рекомендации
          </p>
        </div>
      </div>
    </div>
