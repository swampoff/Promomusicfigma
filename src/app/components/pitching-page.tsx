...
import { catalogPlaylists, catalogStations, catalogInfluencers, catalogMediaOutlets } from '@/data/pitching-catalog';
...
function CampaignsTab({ userCoins, onCoinsUpdate }: { userCoins: number; onCoinsUpdate: (coins: number) => void }) {
  ...
  const { getPitchingsByUser } = useData();
  ...
  const [campaigns, setCampaigns] = useState(
    userPitchings.length > 0
      ? userPitchings.map(p => ({
          id: p.id,
          name: p.campaignName || p.trackTitle || 'Кампания',
          type: p.campaignType || 'track',
          budget: 0,
          spent: 0,
          views: 0,
          likes: 0,
          image: p.coverUrl || 'https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=800'
        }))
      : defaultCampaigns
  );
...
function InfluencersTab({ userCoins, onCoinsUpdate }: { userCoins: number; onCoinsUpdate: (coins: number) => void }) {
  ...
  const influencers = catalogInfluencers;
...
function MediaTab({ userCoins, onCoinsUpdate }: { userCoins: number; onCoinsUpdate: (coins: number) => void }) {
  ...
  const outlets = catalogMediaOutlets;
