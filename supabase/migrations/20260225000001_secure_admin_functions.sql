-- =====================================================
-- SECURITY FIX: Add admin role check to all SECURITY DEFINER functions
-- Prevents any authenticated user from calling admin-only functions
-- =====================================================
...
-- =====================================================
-- GET ADMIN STATS (with role check)
-- =====================================================
  ...
  SELECT json_build_object(
    'users', json_build_object(
      ...
      'by_role', (
        ...
        FROM (SELECT role, COUNT(*) as count FROM users_extended GROUP BY role) t
      )
    ),
...
-- =====================================================
-- GET PENDING MODERATION (with role check)
-- =====================================================
  ...
  SELECT json_build_object(
    'tracks', (
      SELECT json_agg(json_build_object(
        ...
        'duration_seconds', t.duration_seconds, 'created_at', t.created_at,
        'user', json_build_object('id', u.id, 'display_name', u.display_name, 'avatar_url', u.avatar_url)
      ))
      FROM tracks t LEFT JOIN users_extended u ON t.user_id = u.id
    ...
    'videos', (
      ...
      FROM videos v LEFT JOIN users_extended u ON v.user_id = u.id
      WHERE v.status = 'pending' ORDER BY v.created_at DESC LIMIT 50
    ),
    'concerts', (
      SELECT json_agg(json_build_object(
        'id', c.id, 'title', c.title, 'city', c.city, 'venue', c.venue,
        'event_date', c.event_date, 'created_at', c.created_at,
        'user', json_build_object('id', u.id, 'display_name', u.display_name)
      ))
      FROM concerts c LEFT JOIN users_extended u ON c.user_id = u.id
      WHERE c.status = 'pending' ORDER BY c.created_at DESC LIMIT 50
    ),
    'news', (
      SELECT json_agg(json_build_object(
      ...
      FROM news n LEFT JOIN users_extended u ON n.user_id = u.id
      WHERE n.status = 'pending' ORDER BY n.created_at DESC LIMIT 50
    )
  ) INTO result;
  RETURN result;
END;
...
-- =====================================================
-- MODERATE TRACK (with role check)
-- =====================================================
CREATE OR REPLACE FUNCTION moderate_track(
  p_track_id UUID, p_admin_id UUID, p_action TEXT, p_notes TEXT DEFAULT NULL
) RETURNS JSON AS $$
...
BEGIN
  ...
  UPDATE tracks SET
    ...
    moderation_notes = p_notes, moderated_by = p_admin_id, moderated_at = NOW(),
    published_at = CASE WHEN p_action = 'approve' THEN NOW() ELSE NULL END
  WHERE id = p_track_id;

  INSERT INTO notifications (user_id, type, title, message, priority)
  VALUES (v_user_id, 'moderation',
    CASE WHEN p_action = 'approve' THEN 'Трек одобрен' ELSE 'Трек отклонен' END,
    CASE WHEN p_action = 'approve' THEN 'Ваш трек прошел модерацию и опубликован!'
    ELSE 'Ваш трек не прошел модерацию. ' || COALESCE(p_notes, 'Причина не указана.') END,
    CASE WHEN p_action = 'approve' THEN 'normal' ELSE 'high' END);

  SELECT json_build_object('success', true, 'action', p_action, 'track_id', p_track_id) INTO result;
  RETURN result;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- =====================================================
-- MODERATE VIDEO (with role check)
-- =====================================================
CREATE OR REPLACE FUNCTION moderate_video(
  p_video_id UUID, p_admin_id UUID, p_action TEXT, p_notes TEXT DEFAULT NULL
) RETURNS JSON AS $$
DECLARE v_user_id UUID; result JSON;
BEGIN
  IF NOT is_admin() THEN
  ...
  VALUES (v_user_id, 'moderation',
    ...
    CASE WHEN p_action = 'approve' THEN 'Ваше видео прошло модерацию!'
    ELSE 'Ваше видео не прошло модерацию. ' || COALESCE(p_notes, '') END);
...
-- =====================================================
-- GET USERS BY ROLE (with role check)
-- =====================================================
CREATE OR REPLACE FUNCTION get_users_by_role(
  p_role TEXT DEFAULT NULL, p_status TEXT DEFAULT NULL,
  ...
  SELECT json_build_object(
    'users', (
      SELECT json_agg(t) FROM (
        SELECT id, username, display_name, email, phone, avatar_url, role, status,
          verified, followers_count, total_plays, balance, coins_balance,
          subscription_tier, last_active_at, created_at
        FROM users_extended
        WHERE (p_role IS NULL OR role = p_role) AND (p_status IS NULL OR status = p_status)
...
-- =====================================================
-- UPDATE USER STATUS (with role check)
-- =====================================================
CREATE OR REPLACE FUNCTION update_user_status(
  p_user_id UUID, p_admin_id UUID, p_status TEXT, p_reason TEXT DEFAULT NULL
) RETURNS JSON AS $$
BEGIN
  IF NOT is_admin() THEN
    RAISE EXCEPTION 'Unauthorized: admin role required';
  ...
  UPDATE users_extended SET status = p_status WHERE id = p_user_id;

  INSERT INTO notifications (user_id, type, title, message, priority)
  VALUES (p_user_id, 'system', 'Изменение статуса аккаунта',
    'Статус вашего аккаунта изменен на: ' || p_status || '. ' || COALESCE(p_reason, ''), 'high');
...
-- =====================================================
-- GET FINANCIAL STATS (with role check)
-- =====================================================
CREATE OR REPLACE FUNCTION get_financial_stats(p_period TEXT DEFAULT 'month')
RETURNS JSON AS $$
DECLARE v_period_start TIMESTAMPTZ; result JSON;
BEGIN
  IF NOT is_admin() THEN
    RAISE EXCEPTION 'Unauthorized: admin role required';
  END IF;
  ...
  SELECT json_build_object(
    'revenue', (SELECT COALESCE(SUM(amount), 0) FROM transactions WHERE type IN ('deposit', 'purchase') AND status = 'completed' AND created_at >= v_period_start),
    'payouts', (SELECT COALESCE(SUM(amount), 0) FROM transactions WHERE type = 'withdrawal' AND status = 'completed' AND created_at >= v_period_start),
    'pending_payouts', (SELECT COALESCE(SUM(amount), 0) FROM transactions WHERE type = 'withdrawal' AND status = 'pending'),
    'transactions_count', (SELECT COUNT(*) FROM transactions WHERE created_at >= v_period_start),
    'by_type', (
      SELECT json_object_agg(type, total) FROM (
        SELECT type, COALESCE(SUM(amount), 0) as total FROM transactions
        WHERE created_at >= v_period_start AND status = 'completed' GROUP BY type
      ) t
    )
  ) INTO result;
  RETURN result;
...
-- Permissions stay the same — the role check is inside each function now
