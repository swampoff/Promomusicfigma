...
import { deleteBeat, getBeat, getBeatReview, getBeats, upsertBeat, upsertBeatReview, upsertNotification, beatFavoritesStore, beatPurchasesByProducerStore, beatPurchasesByUserStore, beatPurchasesStore, beatReviewsIndexStore, contractsStore, digitalGoodsStore, digitalPurchasesByUserStore, digitalPurchasesStore, platformRevenueLogStore, platformStatsStore, producerServicesStore, serviceOrdersByClientStore, serviceOrdersByProducerStore, serviceOrdersStore } from './db.tsx';
...
async function sendSSE(userId: string, event: string, data: any) {
  try {
    ...
    await upsertNotification(userId, notifId, JSON.stringify({
      id: notifId,
      userId,
      type: event,
...
/** Собрать все публичные услуги из DB */
async function getAllServices(): Promise<any[]> {
  try {
    const entries = await producerServicesStore.getAll();
    if (!entries || entries.length === 0) return [];
    return entries.map((raw: any) => {
      if (typeof raw === 'string') {
        try { return JSON.parse(raw); } catch { return null; }
...
app.get('/beats/favorites', async (c) => {
  try {
    ...
    const raw = await beatFavoritesStore.get(userId);
...
app.get('/beats/purchases', async (c) => {
  try {
    ...
    const raw = await beatPurchasesByUserStore.get(userId);
    const purchaseIds: string[] = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : [];

    const purchases: any[] = [];
    for (const pid of purchaseIds) {
      const p = await beatPurchasesStore.get(pid);
...
app.post('/beats', async (c) => {
  try {
    ...
    // Обновить статистику
    try {
      const statsRaw = await platformStatsStore.get('platform');
      if (statsRaw) {
...
app.post('/beats/:id/purchase', async (c) => {
  try {
    ...
    // Сохранить покупку
    await beatPurchasesStore.set(purchaseId, JSON.stringify(purchase));

    // Индекс покупателя
    const buyerRaw = await beatPurchasesByUserStore.get(buyerId);
    const buyerPurchases: string[] = buyerRaw
      ? (typeof buyerRaw === 'string' ? JSON.parse(buyerRaw) : buyerRaw)
      : [];
    buyerPurchases.unshift(purchaseId);
    await beatPurchasesByUserStore.set(buyerId, JSON.stringify(buyerPurchases));

    // Индекс продюсера
    const prodRaw = await beatPurchasesByProducerStore.get(beat.producerId);
    const prodPurchases: string[] = prodRaw
      ? (typeof prodRaw === 'string' ? JSON.parse(prodRaw) : prodRaw)
      : [];
    prodPurchases.unshift(purchaseId);
    await beatPurchasesByProducerStore.set(beat.producerId, JSON.stringify(prodPurchases));
...
app.post('/beats/:id/favorite', async (c) => {
  try {
    ...
    await beatFavoritesStore.set(userId, JSON.stringify(favs));

    // Обновить likes на бите
    try {
      const beatRaw = await getBeat(beatId);
      if (beatRaw) {
        const beat = typeof beatRaw === 'string' ? JSON.parse(beatRaw) : beatRaw;
        beat.likes = Math.max(0, (beat.likes || 0) + (added ? 1 : -1));
        await upsertBeat(beatId, JSON.stringify(beat));
      }
    } catch {}
...
app.get('/beats/:id/reviews', async (c) => {
  try {
    ...
    const indexRaw = await beatReviewsIndexStore.get(beatId);
    ...
    for (const rid of paged) {
      const r = await getBeatReview(beatId, rid);
...
app.post('/beats/:id/review', async (c) => {
  try {
    ...
    const purchasesRaw = await beatPurchasesByUserStore.get(userId);
    ...
    for (const rid of reviewIds) {
      const existing = await getBeatReview(beatId, rid);
      if (existing) {
    ...
    await upsertBeatReview(beatId, reviewId, JSON.stringify(review));

    // Обновить индекс (новые сверху)
    reviewIds.unshift(reviewId);
    await beatReviewsIndexStore.set(beatId, JSON.stringify(reviewIds));
...
app.post('/beats/:id/reviews/:reviewId/helpful', async (c) => {
  try {
    ...
    const raw = await getBeatReview(beatId, reviewId);
...
app.get('/services/:id', async (c) => {
  try {
    const id = c.req.param('id');
    const raw = await producerServicesStore.get(id);
...
app.post('/services/:id/order', async (c) => {
  try {
    ...
    const raw = await producerServicesStore.get(serviceId);
    if (!raw) return c.json({ success: false, error: 'Service not found' }, 404);
    const service = typeof raw === 'string' ? JSON.parse(raw) : raw;
    ...
    await serviceOrdersStore.set(orderId, JSON.stringify(order));

    // Индекс клиента
    const clientRaw = await serviceOrdersByClientStore.get(clientId);
    const clientOrders: string[] = clientRaw
      ? (typeof clientRaw === 'string' ? JSON.parse(clientRaw) : clientRaw)
      : [];
    clientOrders.unshift(orderId);
    await serviceOrdersByClientStore.set(clientId, JSON.stringify(clientOrders));

    // Индекс продюсера
    const prodRaw = await serviceOrdersByProducerStore.get(service.producerId);
    const prodOrders: string[] = prodRaw
      ? (typeof prodRaw === 'string' ? JSON.parse(prodRaw) : prodRaw)
      : [];
    prodOrders.unshift(orderId);
    await serviceOrdersByProducerStore.set(service.producerId, JSON.stringify(prodOrders));
...
app.get('/orders', async (c) => {
  try {
    ...
    const raw = role === 'producer'
      ? await serviceOrdersByProducerStore.get(userId)
      : await serviceOrdersByClientStore.get(userId);
    const orderIds: string[] = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : [];

    const orders: any[] = [];
    for (const oid of orderIds) {
      const o = await serviceOrdersStore.get(oid);
...
app.put('/orders/:id/status', async (c) => {
  try {
    ...
    const raw = await serviceOrdersStore.get(orderId);
    ...
    if (status === 'completed') {
      ...
      try {
        const svcRaw = await producerServicesStore.get(order.serviceId);
        if (svcRaw) {
          const svc = typeof svcRaw === 'string' ? JSON.parse(svcRaw) : svcRaw;
          svc.orders = (svc.orders || 0) + 1;
          await producerServicesStore.set(order.serviceId, JSON.stringify(svc));
        }
      } catch {}

      // Записать комиссию платформы при прямом завершении (без контракта)
      const existingContract = await contractsStore.get(orderId);
      if (!existingContract) {
        await recordPlatformCommission(
          'service',
          order.serviceTitle,
...
app.post('/orders/:id/contract', async (c) => {
  try {
    ...
    const orderRaw = await serviceOrdersStore.get(orderId);
    ...
    const existingRaw = await contractsStore.get(orderId);
    if (existingRaw) {
    ...
    await contractsStore.set(orderId, JSON.stringify(contract));
...
app.get('/contracts/:id', async (c) => {
  try {
    const orderId = c.req.param('id');
    const raw = await contractsStore.get(orderId);
...
app.put('/contracts/:id/milestones/:mid', async (c) => {
  try {
    ...
    const orderRaw2 = await serviceOrdersStore.get(orderId);
    if (orderRaw2) {
...
async function getAllDigitalGoods(): Promise<any[]> {
  try {
    const entries = await digitalGoodsStore.getAll();
    if (!entries || entries.length === 0) return [];
    return entries.map((raw: any) => {
      if (typeof raw === 'string') {
        try { return JSON.parse(raw); } catch { return null; }
...
app.get('/digital-goods/:id', async (c) => {
  try {
    const id = c.req.param('id');
    const raw = await digitalGoodsStore.get(id);
...
app.post('/digital-goods/:id/purchase', async (c) => {
  try {
    ...
    await digitalPurchasesStore.set(purchaseId, JSON.stringify(purchase));

    // Индексы
    const buyerRaw = await digitalPurchasesByUserStore.get(buyerId);
    const buyerList: string[] = buyerRaw ? (typeof buyerRaw === 'string' ? JSON.parse(buyerRaw) : buyerRaw) : [];
    buyerList.unshift(purchaseId);
    await digitalPurchasesByUserStore.set(buyerId, JSON.stringify(buyerList));
...
app.get('/admin/transactions', async (c) => {
  try {
    const raw = await platformRevenueLogStore.get('singleton');
