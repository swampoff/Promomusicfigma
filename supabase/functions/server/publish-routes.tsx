...
const PRICING = {
  video: {
    ...
    premium: { price: 2990, label: 'Премиум', features: ['Публикация на платформе', 'Продвижение в ленте + баннер', 'Полная аналитика', 'Модерация 4ч', 'Топ рекомендаций', 'Публикация в соцсетях Promo.music', 'Рассылка подписчикам'] },
  },
  concert: {
    ...
    premium: { price: 4990, label: 'Премиум', features: ['Страница события', 'Баннер на главной', 'Продажа билетов', 'Модерация 4ч', 'Push + email рассылка', 'Публикация в Promo.guide', 'Таргетированное продвижение'] },
  },
};
...
async function sendCriticalEmailNotification(
  ...
  try {
    ...
    const statusLabel = STATUS_LABELS_RU[newStatus] || newStatus;
    const subject = `Promo.music - ваша публикация ${statusLabel}`;
    const content = [
